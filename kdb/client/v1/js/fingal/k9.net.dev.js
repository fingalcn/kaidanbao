	...
	k.net={
		api:function(api_path,param,comp,do_not_need_session){
			if(do_not_need_session || k.cache.sign.session){
				...
				ajax({url:api_path,data:JSON.stringify({s:k.cache.sign.session || {},p:param})},function(err,r){
					console.log(api_path+':param',param);
					console.log(api_path+':result',r);
					if(err){
						console.log(api_path+':error',err);
						comp(err);
					}else if(r.code === 200){
						comp(null,r);
					}else{
